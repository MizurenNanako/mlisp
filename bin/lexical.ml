...
module Token = struct
  ...
  type token =
    | Tterm of string ranged
    | Tstring of string ranged
    | Tint of int64 ranged
    | Treal of float ranged
    | Tbool of bool ranged
  ...
  let to_string (t : token) =
    ...
    | LP _ -> "("
    | RP _ -> ")"
    | Tterm (str, r) -> str ^ Range.str r
    | Tstring (str, r) -> str ^ Range.str r
    | Tint (i, r) -> Int64.to_string i ^ Range.str r
    | Treal (a, r) -> Printf.sprintf "%f" a ^ Range.str r
    | Tbool (b, r) -> Printf.sprintf "%b" b ^ Range.str r
    | QUOTE _ -> "\'"
    | CXR (l, r) ->
        (l
        |> List.map (fun c ->
